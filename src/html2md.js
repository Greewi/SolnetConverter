...
exports.HTML2MD = class {
    
    /**
     * Converti un article depuis le format HTML vers un format MarkDown
     * @param {string} htmlSource le nom du fichier HTML à charger
     * @param {string} mdOutput le nom du fichier MarkDown à sauvegarder
     * @returns {Promise}
     */
    static convert(htmlSource, mdOutput){
        let promise = Promise.resolve()
        .then(()=>{
            return io.readFile(htmlSource);
        })
        .then((html)=>{
            let markdown = html;
            ...
            markdown = markdown.replace(/ *<blockquote>(.*?)<\/blockquote> */gms, (correspondance, inner)=>{
                return inner.replace(/\s*(.+)\s*(<br\/>)*\s*/g, "> $1\n");
            });
            markdown = markdown.replace(/ *<div class="encart">(.*?)<\/div> */gms, (correspondance, inner)=>{
            ...
            return io.writeFile(markdown, mdOutput);
        });
        return promise;
    }
